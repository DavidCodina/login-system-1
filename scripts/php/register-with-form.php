...
if ( $_SERVER['REQUEST_METHOD'] != 'POST' ){
  ...
  header("Location: ../../403.php?error=register_with_form_permission_error");
  exit();
    ...
    if ( mysqli_connect_error() ){
      ...
      if ( empty($email) ){
        ...
        if( !mysqli_stmt_prepare($stmt, $sql) ){
          ...
          $_SESSION['form_data']          = [ "first" => $first, "last" => $last, "email" => $email, "username" =>  $username ];
          $_SESSION['registration_data_creation_time'] = time();

          if ( isset($_SESSION['form_errors']) && count($formErrors) === 0) {
      ...
      //Only set $_SESSION['form_errors'] if $formErrors is not empty.
      if ( isset($_SESSION['form_errors']) && count($formErrors) === 0) {
        unset($_SESSION['form_errors']);
      } else if (!empty($formErrors)){
        $_SESSION['form_errors'] = $formErrors;
